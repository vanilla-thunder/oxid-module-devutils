...
<table class="striped">
    ...
    <tr>
        ...
        <th>
            ...
            <span [{if $aModule.version != $oView->getModuleVersion($oView->getEditObjectId())}]class="green-text"[{/if}]>[{$aModule.version}]</span>
        </th>
    </tr>
    </thead>
    <tbody>
    ...
    [{* extensions *}]
    [{assign var="aExtensions" value=$oModule->getExtensions()}]
    ...
    [{if $aControllers || $aModule.controllers}]
        <tr>
            <th colspan="3" class="[{if $aModule.controllers == $aControllers}]green[{else}]red[{/if}]-text">controllers</th>
        </tr>
        <tr>
            <td>
